...
async function verifyToken(req, res, next) {
    ...
    if (!authHeader.startsWith("bearer eg1~")) return authErr();

    const token = (req.headers["authorization"]).replace(/^bearer eg1~/i, "");
...
async function verifyClient(req, res, next) {
    ...
    const authHeader2 = (req.headers["authorization"] || "").toLowerCase();
    if (!authHeader2.startsWith("bearer eg1~")) return authErr();
