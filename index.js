...
mongoose.connect(config.mongodb.database).then(() => {
    ...
    log.error("MongoDB failed to connect. Make sure MongoDB is installed and running.");
    throw err;
});

app.use(rateLimit({ windowMs: 0.5 * 60 * 1000, max: 100 }));
// Raw body for CloudStorage PUT (client settings files are binary)
app.use("/fortnite/api/cloudstorage/user", express.raw({ type: "*/*", limit: "10mb" }));
...
app.listen(PORT, () => {
    log.backend(`Kyro Backend started on port ${PORT}`);
    log.backend(`Discord OAuth login enabled - Client ID: 1502777863890407484`);
